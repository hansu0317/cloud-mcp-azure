...
### 3.2 같은 저장소 안 MCP 서버와의 경계

```mermaid
flowchart LR
    WEB["CRM AI Notebook web runtime<br/>backend/main.py"] -->|"내부 tool calling"| WREST["Dataverse REST/OData"]
    MCPCLIENT["MCP client<br/>(Claude Desktop/Code)"] -->|"JSON-RPC over stdio/streamable-http"| MCPSERVER["backend/mcp_server.py<br/>(별도 프로세스)"]
    MCPSERVER -->|"catalog resource + 2 tools"| MREST["Dataverse REST/OData"]
    WREST -.->|"같은 가드 함수 재사용"| MCPSERVER
```

`backend/mcp_server.py`는 `dataverse://catalog`, `dataverse_describe_table`, `dataverse_query`를 제공하는 MCP 서버다(2026-08-13부터 별도 저장소 `crm-ai-chat-dataverse-mcp`(Node/TS)를 폐기하고 이 저장소 안 Python으로 통합). 웹앱 FastAPI process는 이 MCP process를 실행하거나 호출하지 않는다 — 실행 방식은 여전히 "MCP client가 필요할 때 자식 프로세스로 직접 기동"이라는 독립 배포·설정·로그 대상이다. 다만 이제 물리적으로 같은 저장소·같은 Python 코드베이스에 있어, `backend/chat_api.py`의 provider-neutral 내부 tool contract와 `backend/mcp_server.py`가 **같은 가드 함수**(`_guard_odata_path`, `_describe_table_from_cache`, 8 KiB 상한)를 직접 import해 공유한다 — 예전처럼 Node/TS로 따로 포팅해 두 로직이 갈라질 위험이 없다.
