...
| 문서 | 내용 |
|---|---|
| [아키텍처_도면.html](아키텍처_도면.html) | HANDOVER.md와 같은 순서를 도면 9장으로 정리한 시각 자료 — 전제상황, 코드 계층, 파일 지도, API 스펙, 로그인·질문응답(프로세스·데이터 흐름)·지침·정지버튼·LLM 프로필 전환. 브라우저로 직접 열어서 본다(별도 서버 불필요) |
| [지침_테스트_체크리스트.md](지침_테스트_체크리스트.md) | 지침(조인·용어·예시) 3탭 실사용 검증용 체크리스트 — 2026-08-31 재설계분(용어 True/False 폼, 예시 자동가져오기 제거) 회귀 확인 포함 |
